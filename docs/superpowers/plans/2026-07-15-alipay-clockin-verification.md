...
- [ ] **步骤 2：运行专项测试并确认因方法缺失或返回值为 `None` 而失败**

运行：`python -m unittest tests.test_alipay_clockin_verification.AlipayApiClientTest -v`
...
- [ ] **步骤 4：补充非法深链、缺字段、正常成功和继续 payload 测试并运行**
...
- [ ] **步骤 2：运行任务层测试并确认当前仍误报成功**

运行：`python -m unittest tests.test_alipay_clockin_verification.ClockInTaskResultTest -v`
...
运行：`python -m unittest tests.test_alipay_clockin_verification.ClockInTaskResultTest tests.test_task_runner_makeup_delay -v`
...
- [ ] **步骤 2：运行 API 测试并确认接口和共享函数尚不存在**

运行：`python -m unittest tests.test_alipay_clockin_verification.AlipayContinueApiTest -v`
...
- [ ] **步骤 4：运行继续 API 测试和 OpenAPI 合同测试**

运行：`python -m unittest tests.test_alipay_clockin_verification.AlipayContinueApiTest tests.test_openapi_contract -v`
...
- [ ] **步骤 5：运行静态测试、前端质量门和构建**

运行：

```powershell
python -m unittest tests.test_frontend_alipay_verification -v
Set-Location web
npm run lint
npm test
npm run build
...
```powershell
python -m unittest tests.test_alipay_clockin_verification tests.test_frontend_alipay_verification -v
...
npm run build
Set-Location ..
docker build --build-arg DOWNLOAD_MODELS=0 -t automoguding-saas:alipay-verification-test .
```

预期：审计无 high/critical 漏洞，lint/test/build 退出码为 0，Docker 镜像构建成功。
...
git status --short
git diff --stat
git diff -- server/coreApi/MainLogicApi.py server/task_runner.py server/api.py web/src/components/AlipayVerificationDialog.vue web/src/views/user/UserHome.vue web/src/views/UserList.vue tests/test_alipay_clockin_verification.py tests/test_frontend_alipay_verification.py
git add docs/superpowers/plans/2026-07-15-alipay-clockin-verification.md server/coreApi/MainLogicApi.py server/task_runner.py server/api.py web/src/components/AlipayVerificationDialog.vue web/src/views/user/UserHome.vue web/src/views/UserList.vue tests/test_alipay_clockin_verification.py tests/test_frontend_alipay_verification.py
