...
| 打卡设置 | 代维护时间、地点、周期、图片、备注；处理安全验证 | 自助维护个人配置；验证后显式继续打卡 | 保存后重建调度任务 |
| 打卡预授权 | 按用户和日期代为授权 | 按日期逐项授权 | `304` 后原子消费并最多重试 1 次 |
...
| `/users/:id/preauthorizations` | 管理端预授权页 | 需要 `tasks:run` 权限 |
| `/audit` | 审计日志页 | 关键操作记录 |
...
| `/u/preauthorizations` | 用户预授权页 | 未来上 / 下班和过去补卡预授权 |
...
| `server/clockin_preauthorization.py` | 预授权日期、URL、票据、持久化状态和原子消费 |
...
打卡接口返回 HTTP 200 且业务字段为 `code == 200`、`msg == "304"` 时，状态是 `verification_required`，不能记录为打卡成功。系统优先使用用户提前确认的预授权；没有可用预授权时，才进入现有即时验证流程。

### 日期和状态

动态列表范围为 `User.created_at` 的北京时间日期到实习计划 `planInfo.endTime`，并只生成打卡设置中启用的星期。

| 日期范围 | 类型 | 数量 | 使用方式 |
|----------|------|------|----------|
| 今天及未来 | `START`、`END` | 每天 2 项 | 分别用于上班和下班正常打卡 |
| 已过去日期 | `MAKEUP` | 每天 1 项 | 可用于当日上班或下班补卡，但只能成功占用 1 次 |

状态含义：

| 状态 | 含义 | 是否可重新授权 |
|------|------|----------------|
| `pending` | 尚未确认授权，数据库无记录 | 是 |
| `authorized` | 已确认并可供任务消费 | 否 |
| `consumed` | 已被一次打卡或补卡原子占用 | 是 |
| `reauthorize_required` | 携带凭据重试后仍返回 `304` | 是 |

修改星期、上下班时间或计划结束日会改变当前动态列表，不会主动删除数据库中的历史预授权记录。列表只展示当前有效日期范围内的记录。

### 页面授权流程

1. 用户在管理端或用户端选择日期和类型，调用开始授权接口。
2. 后端创建工学云登记，返回 30 分钟签名票据、`alipays://` 深链和 `https://ds.alipay.com/?scheme=...` 浏览器链接，此时不写数据库。
3. 后端使用结构化 URL 解析，把 `thirdPartSchema` 替换为 `https://fanyi.baidu.com/m/trans?from=zh&to=en&query=...`。`query` 文本包含当前 `User.phone` 和登记发起的北京时间。
4. 用户用任一方式打开支付宝并完成授权。
5. 用户返回页面点击「我已完成授权」，前端只提交签名票据。
6. 后端验证用途、租户、用户、日期、类型和有效期后，幂等保存预授权记录。

`outRegisterNo` 按业务要求明文保存；`registerUrl` 和签名票据不持久化。关闭授权对话框会清空前端内存中的临时数据。
...
| 首次打卡 | 不带任何预授权凭据 |
| 首次成功 | 不查询、不占用预授权 |
| 首次 `304` 且有匹配凭据 | 原子更新为 `consumed`，携带登记编号重试 1 次 |
| 第二次仍为 `304` | 更新为 `reauthorize_required`，不发起第 3 次请求 |
| 首次 `304` 且无凭据 | 普通打卡创建即时验证登记；补卡返回需要验证的失败结果 |
| 并发执行 | 条件更新保证定时任务和手动任务只有一方取得同一凭据 |

即时验证由用户点击「验证完成，继续打卡」后显式调用：

```json
{
  "out_register_no": "example-register-no",
  "target_type": "START"
}
```

`out_register_no` 必填，`target_type` 只允许 `START` 或 `END`。继续接口只提交 1 次，不读取预授权。

### 预授权 API

| 操作 | 用户端 | 管理端 |
|------|--------|--------|
| 列表 | `GET /app/clock-in/preauthorizations` | `GET /users/{user_id}/clock-in/preauthorizations` |
| 开始 | `POST /app/clock-in/preauthorizations/start` | `POST /users/{user_id}/clock-in/preauthorizations/start` |
| 完成 | `POST /app/clock-in/preauthorizations/complete` | `POST /users/{user_id}/clock-in/preauthorizations/complete` |
| 即时验证后继续 | `POST /app/clock-in/alipay/continue` | `POST /users/{user_id}/clock-in/alipay/continue` |

管理端 3 个预授权接口和即时继续接口都要求 `tasks:run`。用户端只能操作当前登录账号绑定的工学云用户。

### 数据边界

| 数据 | 持久化 | 可进入 API 列表、日志、审计详情、通知或执行历史 |
|------|--------|--------------------------------------------------|
| `outRegisterNo` | 用户确认后明文保存到预授权表 | 否 |
| `registerUrl` | 否 | 否 |
| `registration_ticket` | 否 | 仅开始响应和完成请求；票据封装登记编号，签名防篡改但不加密，并限制用途和 30 分钟有效期 |
| 日期、类型、状态、授权时间 | 是 | 可进入预授权列表和脱敏审计详情 |
...
Docker Publish 在镜像构建前依次运行 action 固定策略、Python 依赖审计、数据库迁移、后端测试、编译、质量门、备份恢复、前端审计与构建，以及 Trivy 文件系统扫描。任一前置步骤失败时，Docker Buildx、镜像推送、签名和验签不会执行。
